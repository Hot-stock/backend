...
class NewsServiceTest {
	...
	void 주식의_상승이유를_찾을_수_있을_떄_GPTNEWDomain객체를_반환() {
		...
		when(gptAPIPort.findStockRaiseReason(anyString(), anyString(), any())).thenReturn(
			Optional.of(GPTStockNewsDomain));
	...
	void 주식의_상승_이유가_여러_개여서_getNew가_여러개를_반환해야함() {
		...
		ohlc.addRoseNews(GPTStockNewsDomain2);
		...
		List<GPTStockNewsDomain> raiseReasonThatDate = newsService.findRaiseReason(stockName, LocalDate.now());
