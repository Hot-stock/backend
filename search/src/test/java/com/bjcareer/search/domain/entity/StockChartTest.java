...
class StockChartTest {
	...
	void 주어진_날의_뉴스를_가지고_옴() {
		...
		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.loadNewByDate(targetDate);
		assertEquals(2, GPTStockNewsDomains.size());
	}

	@Test
	void 주어진_날의_뉴스가_없을때() {
		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.loadNewByDate(LocalDate.now().plusDays(1));
		assertTrue(GPTStockNewsDomains.isEmpty());
	}
	...
	void 주어진_날짜에_뉴스가_없을_때() {
		...
		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.loadNewByDate(targetDate);
		assertEquals(0, GPTStockNewsDomains.size());
	}
	...
	void 뉴스는_있지만_다음_일정이_없는_뉴스들_일떄() {
		LocalDate targetDate = LocalDate.now();
		addNewsToOhldUsingDate(targetDate);
		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.getNextSchedule(targetDate);
		assertEquals(0, GPTStockNewsDomains.size());
	}
	...
	void 뉴스는_있지만_다음_일정이_있는_뉴스들() {
		...
		ohlcList.getFirst().addRoseNews(
			new GPTStockNewsDomain("퓨런티어", "트럼프 당선이 예상되면서 테슬라가 올라서 같이 올랐다", new ArrayList<>(), nextSchedulerDate.toString(), null));

		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.getNextSchedule(LocalDate.now());
		assertEquals(1, GPTStockNewsDomains.size());
	}
	...
	void 모든_뉴스를_가지고_옴() {
		...
		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.getAllNews();

		assertEquals(4, GPTStockNewsDomains.size());
	}
	...
	void 주어진_날짜에_뉴스를_링크할_수_있는지() {
		Map<String, String> themas = Map.of("자율주행", "트럼프가 좋아함");
		GPTStockNewsDomain GPTStockNewsDomain = new GPTStockNewsDomain("퓨런티어", "트럼프 당선이 예상되면서 테슬라가 올라서 같이 올랐다", new ArrayList<>(), null, null);
		this.chart.addNewsToOhlc(GPTStockNewsDomain, LocalDate.now());

		List<GPTStockNewsDomain> GPTStockNewsDomains = this.chart.loadNewByDate(LocalDate.now());
	...
	private void addNewsToOhldUsingDate(LocalDate date) {
		...
		for (OHLC ohlc : ohlcList) {
			if (ohlc.getDate().isEqual(date)) {
				ohlc.addRoseNews(new GPTStockNewsDomain("퓨런티어", "트럼프 당선이 예상되면서 테슬라가 올라서 같이 올랐다", new ArrayList<>(), null, null));
				ohlc.addRoseNews(new GPTStockNewsDomain("퓨런티어", "트럼프 당선 확률이 높아지면서 올랐다", new ArrayList<>(), null, null));
			}
		}
	}
}
