...
class OHLCTest {
	...
	private final GPTStockNewsDomain GPTStockNewsDomain = new GPTStockNewsDomain("배럴", "휴가로 인해서 래쉬가드 수요가 증가함", new ArrayList<>(), null, null);
	...
	void ohlc에_여러_개의_뉴스가_등록되어_List형식으로_변환됨(){
		...
		ohlc.addRoseNews(GPTStockNewsDomain); //중복된 뉴스지만 저장됨
		ohlc.addRoseNews(GPTStockNewsDomain);
