...
public class RegisterController {
    ...
    @Operation(summary = "이메일 인증 토큰 생성", description = "사용자가 입력한 이메일로 회원가입 인증을 위한 랜덤 토큰을 생성하고 전송합니다. 중복된 이메일이 있으면 에러를 반환합니다.")
    ...
    @Operation(summary = "이메일 인증 토큰 검증", description = "사용자가 입력한 이메일로 발송된 토큰을 검증합니다.")
    @HasRole(RoleType.ALL)
    ...
    @PostMapping
    @Operation(summary = "회원가입", description = "사용자가 입력한 이메일 및 패스워드로 회원가입을 진행합니다.")
    @HasRole(RoleType.ALL)
